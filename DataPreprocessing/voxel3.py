...
parent_dir = '/home/wt/RadHAR/Data/Train'
sub_dirs=['swing']
...
def get_data(file_path):
    ...
    for i in range(0,length1):
        ...
        if wordlist[i] == "doppler_bin:":
            doppler_bin.append(wordlist[i+1])
    ...
    doppler_bin = np.asarray(doppler_bin)
    ...
    doppler_bin = doppler_bin.astype(np.float)
    ...
    for i in range(len(frame_num)):
        if int(frame_num[i]) in data:
            ...
            data[frame_num[i]].append([x[i],y[i],z[i],doppler_bin[i]])
        else:
            data[frame_num[i]]=[]
            #data[frame_num[i]].append([x[i],y[i],z[i],velocity[i],intensity[i]])
            data[frame_num[i]].append([x[i],y[i],z[i],doppler_bin[i]])
    ...
    del x,y,z, doppler_bin, data, data_pro1, pixels
